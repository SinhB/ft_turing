...
from utils.error import NoTransitionError, NoSolutionError
...
def perform_transition(
    ...
    if len(transition) != 1:
        raise NoTransitionError(f"Expected 1 transition for state {current_state}" + f" with read {tape[head]}, got {len(transition)}")
